...
class BackgroundDataset(data.Dataset, datasets.imdb):
    ...
    def __init__(self, name):
        ...
        if name == 'coco':
            ...
            self.depth_factor = 10000.0

            # read file list
            cache_file = os.path.join(self.cache_path, '../', 'sunrgbd.pkl')
            with open(cache_file, 'rb') as fid:
                files = cPickle.load(fid)

            for i in range(len(files)):
                # depth image
                dirname_depth = os.path.join(self.cache_path, '../', files[i])
                images = os.listdir(dirname_depth)
                filename = os.path.join(dirname_depth, images[0])
                self.files_depth.append(filename)

                # rgb image
                dirname = dirname_depth.replace('depth', 'image')
                images = os.listdir(dirname)
                filename = os.path.join(dirname, images[0])
                self.files_color.append(filename)

                # intrinsics
                filename = os.path.join(dirname_depth, '..', 'intrinsics.txt')
                K = np.loadtxt(filename).reshape((3, 3))
                self.intrinsics.append(K)

            print('%d rgb images' % (len(self.files_color)))
            print('%d depth images' % (len(self.files_depth)))
            print('%d intrinsics' % (len(self.intrinsics)))
    ...
    def __getitem__(self, idx):
        ...
        if self.name == 'rgbd' or self.name == 'sunrgbd':
            filename_depth = self.files_depth[idx]
            if len(self.intrinsics) > 0:
                intrinsics = self.intrinsics[idx]
            else:
                intrinsics = self._intrinsic_matrix
        else:
            filename_depth = None
            intrinsics = None
        return self.load(filename_color, filename_depth, intrinsics)
        ...
        if filename_depth is None and np.random.rand(1) < cfg.TRAIN.SYN_BACKGROUND_CONSTANT_PROB:  # only for rgb cases
            ...
            try:
                ...
                if filename_depth is not None:
                    ...
                    background_depth = self.backproject(background_depth, intrinsics, self.depth_factor)
